...
import models
import datasets
from loss import compute_semantic_pos_loss,compute_segment_loss
import datetime
...
#python main2lr.py  --data='F:\dhw\code\FCNDATA' --savepath=pretrain_ckpt --workers=0 -b=16 --pretrained=F:\dhw\code\vip1withg\pretrain_ckpt\BSD500\SpixelNet1l_bn_adam_3000000epochs_epochSize6000_b16_lr0.0005_posW0.003_24_12_12_19_35\checkpoint.tar
#tensorboard --logdir=C:\Users\DELL\Desktop\logs\20241231 --port=1235
#python mainkaglr.py  --data='/HOME/scw6eia/run/data/DATA' --savepath=pretrain_ckpt --workers=0 -b=8 --lr=0.0005
...
def main():
    ...
    #预训练加载
    #checkpoint = torch.load(r'F:\dhw\code\vip1withg\pretrain_ckpt\BSD500\SpixelNet1l_bn_adam_3000000epochs_epochSize6000_b16_lr0.0005_posW0.003_24_12_13_17_43\checkpoint.tar')
    ...
    save_path_SW = "logs/" + datetime.datetime.now().strftime("%Y%m%d")
    train_writer = SummaryWriter(os.path.join(save_path_SW, 'train'))
    val_writer = SummaryWriter(os.path.join(save_path_SW, 'val'))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # if epoch > 450:
        #     save_checkpoint(rec_dict, is_best=False, filename='%d_epoch.tar' % epoch)
...
def train(train_loader, model, optimizer, epoch, train_writer, init_spixl_map_idx, xy_feat):
    ...
    for i, (input, label) in enumerate(train_loader):
        ...
        output1, output2 = model(input_gpu)
        slic_loss, loss_sem, loss_pos = compute_semantic_pos_loss(output1, LABXY_feat_tensor,
                                                                pos_weight= args.pos_weight, kernel_size=args.downsize)
        seg_loss, _, _ = compute_semantic_pos_loss(output2, LABXY_feat_tensor,
                                                                pos_weight= args.pos_weight, kernel_size=args.downsize)
        allloss = slic_loss + seg_loss*0.1
        # ========= back propagate ===============
        optimizer.zero_grad()
        ...
        losses_seg.update(seg_loss.item(), input_gpu.size(0))

        if i % args.print_freq == 0:
            print('train Epoch: [{0}][{1}/{2}]\t Time {3}\t Data {4}\t Total_loss {5}\t Loss_sem {6}\t Loss_pos {7}\t Loss_seg {8}\t'
                  .format(epoch, i, epoch_size, batch_time, data_time, total_loss, losses_sem, losses_pos,losses_seg))
    ...
    if epoch % args.record_freq == 0:
        ...
        train_writer.add_scalar('loss_seg', seg_loss.item(), epoch)
        ...
        curr_spixl_map = update_spixl_map(init_spixl_map_idx,output1)
        ...
        curr_spixl_map1 = update_spixl_map(init_spixl_map_idx, output2)
        spixel_lab_save1 = make_grid(curr_spixl_map1, nrow=args.batch_size)[0, :, :]
        spixel_viz1, _ = get_spixel_image(input_l_save, spixel_lab_save1)
        train_writer.add_image('Spixel viz1111', spixel_viz1, epoch)
