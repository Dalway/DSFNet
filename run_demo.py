...
def test(args, model, img_paths, save_path, idx):
    ...
    #curr_spixl_map = spixeIds
    #output=torch.ones(output.shape).cuda()
    ...
    #print((spixel_viz.transpose(1, 2, 0)).name)

    #print(np.max(spixel_viz.transpose(1, 2, 0)))
    print(type(spixel_viz))
